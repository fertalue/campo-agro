...
function FilaCatalogo({ prod, onSave, onDelete }) {
  ...
  async function handleBuscarPrecio() {
    ...
    setPreciosSug(data); setShowPrecios(true)
    ...
    <div style={{ padding:'12px 14px', marginBottom:6, background:'#FFF9EE', border:'1px solid #C8A96E', borderRadius:10 }}>
      ...
      <div style={{ marginBottom:8 }}>
        <button type="button" onClick={handleBuscarPrecio} disabled={buscandoPrecio}
          ...
          {buscandoPrecio ? '⏳' : '💰 Ver precio en costos'}
        </button>
          ...
          <div style={{ marginTop:6, display:'flex', flexDirection:'column', gap:3 }}>
            {preciosSug.map((c,i)=>(
              <div key={i} style={{ display:'flex', alignItems:'center', gap:8, background:'white', borderRadius:5, padding:'5px 8px', border:'1px solid #E8D5A3', fontSize:11 }}>
                <span style={{ color:'var(--text-muted)' }}>{c.fecha}</span>
                <span style={{ flex:1, color:'var(--tierra)', fontWeight:500 }}>{c.producto_servicio}</span>
                <span style={{ color:'var(--arcilla)' }}>{c.proveedor}</span>
                <div style={{ textAlign:'right' }}>
                  {c.precio_unitario_sin_iva_usd
                    ? <div style={{ fontWeight:700, color:'#2E4F26' }}>U$S {fmtNum(c.precio_unitario_sin_iva_usd,3)}/{c.unidad_base||c.unidad||'u'}</div>
        ...
        {showPrecios && preciosSug.length === 0 && <span style={{ fontSize:11, color:'var(--text-muted)', marginLeft:8 }}>Sin facturas encontradas</span>}
      </div>
...
function FormMovimiento({ tipo, productos, costos, quienRegistra, onSave, onCancel }) {
  ...
  const f = (k,v) => setForm(p => {
    ...
    if (k==='costo_id' && v) {
      // Auto-completar precio desde la factura de costos
      const costo = costos.find(c=>c.id===v)
      if (costo?.precio_unitario_sin_iva_usd) {
        ...
        if (costo.proveedor) next.proveedor = costo.proveedor
      }
    }
    return next
  })
  ...
  return (
    <div className="card mb-3" style={{background:tipo==='compra'?'#F0F7EE':'#E4F0F4',borderColor:tipo==='compra'?'#9DC87A':'#7A9EAD'}}>
      <h3 style={{marginBottom:14}}>{tipo==='compra'?'Remito de compra':'Ingreso de stock inicial'}</h3>
      <form onSubmit={save} style={{display:'flex',flexDirection:'column',gap:12}}>
        ...
        {isCompra && (
          <div className="field">
            <label className="label">Factura relacionada <span style={{fontWeight:400,color:'var(--text-muted)'}}>— al vincularla se auto-completa el precio</span></label>
            <select style={si} value={form.costo_id} onChange={e=>f('costo_id',e.target.value)}>
              <option value="">— Sin factura por ahora —</option>
              {costos.slice(0,60).map(c=>(
                <option key={c.id} value={c.id}>
                  {c.fecha} · {c.proveedor} · {c.producto_servicio}
                  {c.precio_unitario_sin_iva_usd?' → U$S '+Number(c.precio_unitario_sin_iva_usd).toFixed(3)+'/u':''}
                </option>
              ))}
            </select>
          </div>
...
export default function Almacen() {
  ...
  async function vincularFactura(movId, costoId) {
    const updates = { costo_id: costoId || null }
    // Auto-completar precio si la factura tiene precio unitario
    if (costoId) {
      const costo = costos.find(c => c.id === costoId)
      if (costo?.precio_unitario_sin_iva_usd) {
        updates.precio_unitario = costo.precio_unitario_sin_iva_usd
        const mov = movs.find(m => m.id === movId)
        if (mov?.cantidad) updates.precio_total = mov.cantidad * costo.precio_unitario_sin_iva_usd
        if (costo.proveedor && !mov?.proveedor) updates.proveedor = costo.proveedor
      }
    }
    await supabase.from('almacen_movimientos').update(updates).eq('id', movId)
    await fetchAll()
  }
  ...
  movs.forEach(m => {
    ...
    if (!stockPorProducto[key]) stockPorProducto[key] = {
      ...
      cantidad:0, movs:[], producto_id:m.producto_id
    }
    ...
    <div>
      <div className="flex-between mb-2">
        <div>
          ...
          <p style={{fontSize:12,color:'var(--arcilla)',marginTop:2}}>
            ...
            {valorTotalStock > 0 && <> · Valor stock: <strong style={{color:'#2E4F26'}}>U$S {fmtNum(valorTotalStock,0)}</strong></>}
          </p>
        </div>
        {canEdit && tab==='stock' && (
          <div style={{display:'flex',gap:6}}>
          ...
          {valorTotalStock > 0 && (
            <div style={{background:'#EBF4E8',border:'1px solid #9DC87A',borderRadius:10,padding:'12px 16px',marginBottom:14,display:'flex',gap:20,flexWrap:'wrap'}}>
              <div>
                ...
                <div style={{fontSize:11,color:'#4A7C3F'}}>Precio promedio ponderado por producto</div>
              </div>
              <div style={{display:'flex',gap:12,alignItems:'center',flexWrap:'wrap'}}>
                ...
                {stockRows.filter(r=>r.valorStock>0).length > 4 && (
                  <div style={{fontSize:11,color:'var(--text-muted)'}}>+{stockRows.filter(r=>r.valorStock>0).length-4} más</div>
                )}
              </div>
            </div>
          )}
                      ...
                      <div style={{textAlign:'right'}}>
                        {r.precioMedio && (
                          <div style={{fontSize:11,color:'var(--text-muted)'}}>
                            U$S {fmtNum(r.precioMedio,3)}/{r.unidad}
                          </div>
                        )}
                        {r.valorStock && r.cantidad > 0 && (
                          <div style={{fontSize:13,fontWeight:600,color:'#2E4F26'}}>
                            U$S {fmtNum(r.valorStock,0)}
                          </div>
                        )}
                        {r.precioMedio && !r.valorStock && r.cantidad <= 0 && (
                          <div style={{fontSize:11,color:'var(--text-muted)'}}>Ú$S {fmtNum(r.precioMedio,3)}/{r.unidad}</div>
                        )}
                      </div>
                    </div>
                  </div>
            ...
            <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
              <thead>
                <tr style={{background:'#EDE0C8'}}>
                  {['Fecha','Tipo','Producto · Marca','Cantidad','P. unitario','Total USD','Proveedor','Factura / Aplic.'].map(h=>(
              ...
              <tbody>
                {movsFiltrados.map((m,i)=>(
                  <tr key={m.id} style={{borderBottom:'1px solid #EDE0C8',background:i%2===0?'#FDFAF4':'white'}}>
                    ...
                    <td style={{padding:'8px 10px',textAlign:'right',fontFamily:'monospace',fontWeight:600,color:m.tipo==='salida_aplicacion'||m.cantidad<0?'#993C1D':'#2E4F26'}}>
                      {m.cantidad<0?'':m.tipo==='salida_aplicacion'?'-':'+'}{fmtNum(Math.abs(m.cantidad),1)} {m.unidad}
                    </td>
                    ...
                    <td style={{padding:'8px 10px',color:'var(--suelo)',fontSize:11}}>{m.proveedor||'—'}</td>
                    <td style={{padding:'8px 10px'}}>
                      {m.costo_id
                        ...
                        : m.aplicacion_id
                          ...
                          : m.tipo==='compra' && canEdit
                            ? <select onChange={e=>vincularFactura(m.id,e.target.value)}
                                style={{fontSize:10,padding:'2px 4px',border:'1px solid #D8C9A8',borderRadius:4,color:'var(--arcilla)',background:'#FDFAF4',fontFamily:'inherit',maxWidth:140}}>
                                <option value="">Sin factura</option>
                                {costos.slice(0,40).map(c=><option key={c.id} value={c.id}>{c.fecha} · {c.proveedor}</option>)}
                              </select>
      ...
      {tab==='catalogo' && (
        <div>
          ...
          {productos.length===0 && <div style={{padding:32,textAlign:'center',color:'var(--arcilla)'}}>Sin productos. Usá el botón + para agregar.</div>}
        </div>
      )}
    </div>
  )
