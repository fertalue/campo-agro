...
.c-filters{display:flex;flex-wrap:wrap;gap:8px;padding:12px 14px;background:#FDFAF4;border:1px solid #D8C9A8;border-radius:10px;margin-bottom:16px;}
.c-fg{display:flex;flex-direction:column;gap:3px;}
.c-fl{font-size:10px;font-weight:500;color:#A08060;letter-spacing:.05em;text-transform:uppercase;}
...
function fmtk(n) { return n>=1000?`U$S ${(n/1000).toFixed(0)}k`:fmtUSD(n,0) }
...
export default function Costos() {
  ...
  const filtered = costos.filter(c=>{
    ...
    if(fMes!=='Todos'&&monthKey(c.fecha)!==fMes) return false
    if(fNombre!=='Todos'&&c.factura_nombre!==fNombre) return false
    if(fCentro!=='Todos'&&c.centro_costos!==fCentro) return false
    if(fProv!=='Todos'&&c.proveedor!==fProv) return false
    return true
  })
  ...
  return (
    <div>
      ...
      <div className="flex-between mb-2">
        <div>
          ...
          <p style={{fontSize:12,color:'var(--arcilla)',marginTop:2}}>
            {fmtUSD(total)} · {filtered.length} registros {ivaMode==='sin'?'(sin IVA)':'(con IVA)'}
          </p>
        </div>
      ...
      <div className="c-filters">
        <div className="c-fg"><div className="c-fl">Campaña</div>
          <select className="c-sel" value={fCampanha} onChange={e=>setFCampanha(e.target.value)}>
            <option>Todas</option>{CAMPANHAS.map(c=><option key={c}>{c}</option>)}
          </select>
        </div>
        <div className="c-fg"><div className="c-fl">Año / Mes</div>
          <select className="c-sel" value={fMes} onChange={e=>setFMes(e.target.value)}>
            <option value="Todos">Todos</option>{mesesU.map(m=><option key={m} value={m}>{monthLabel(m)}</option>)}
          </select>
        </div>
        <div className="c-fg"><div className="c-fl">Factura a nombre de</div>
          <select className="c-sel" value={fNombre} onChange={e=>setFNombre(e.target.value)}>
            {['Todos','Fer','Leo','ambos','Sin factura'].map(o=><option key={o}>{o}</option>)}
          </select>
        </div>
        <div className="c-fg"><div className="c-fl">Centro de costo</div>
          <select className="c-sel" value={fCentro} onChange={e=>setFCentro(e.target.value)}>
            <option>Todos</option>{CENTROS.map(c=><option key={c}>{c}</option>)}
          </select>
        </div>
        <div className="c-fg"><div className="c-fl">Proveedor</div>
          <select className="c-sel" value={fProv} onChange={e=>setFProv(e.target.value)}>
            <option>Todos</option>{provsU.map(p=><option key={p}>{p}</option>)}
          </select>
        </div>
        <div className="c-fg"><div className="c-fl">Monto USD</div>
        ...
        <div>
          <div className="c-grid2">
            <div className="c-panel">
              ...
              :centros.length===0?<div style={{fontSize:12,color:'var(--arcilla)'}}>Sin datos aún</div>
            ...
            <div className="c-panel">
              ...
              <div style={{display:'flex',alignItems:'center',gap:20}}>
                <svg width="100" height="100" viewBox="0 0 100 100">
                  ...
                  {total>0&&<>
                    <circle cx="50" cy="50" r="36" fill="none" stroke="#6A9E58" strokeWidth="16"
                      strokeDasharray={`${totFer/total*226} 226`} strokeDashoffset="0" transform="rotate(-90 50 50)"/>
                    <circle cx="50" cy="50" r="36" fill="none" stroke="#C8A96E" strokeWidth="16"
                      strokeDasharray={`${totLeo/total*226} 226`} strokeDashoffset={`${-totFer/total*226}`} transform="rotate(-90 50 50)"/>
                  </>}
        ...
        <div className="c-panel" style={{padding:0,overflowX:'auto'}}>
          ...
          :<table className="c-tbl">
            ...
            <tbody>
              {filtered.map(c=>(
                <tr key={c.id}>
                  <td style={{color:'var(--text-muted)',whiteSpace:'nowrap'}}>{c.fecha?new Date(c.fecha+'T12:00:00').toLocaleDateString('es-AR',{day:'2-digit',month:'short'}):'—'}</td>
        ...
        <div>
          <div className="c-panel" style={{marginBottom:14}}>
            ...
            {Object.keys(canjesMes).length===0
              ?<div style={{fontSize:13,color:'var(--arcilla)'}}>No hay canjes registrados</div>
