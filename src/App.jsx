...
// Lazy stubs for modules still in progress
const Placeholder = ({ title }) => (
  <div>
    <h2 style={{ marginBottom: 8 }}>{title}</h2>
    <p style={{ color: 'var(--arcilla)', fontSize: 13 }}>Módulo en construcción. Conectá Supabase y estará listo.</p>
  </div>
)
...
function AppShell() {
  ...
  const PageComponent = PAGES[page] || PAGES.inicio
    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        ...
        <header style={{
          ...
          background: 'var(--blanco)', borderBottom: '1px solid var(--border)',
          flexShrink: 0, gap: 12
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <button onClick={() => setSidebarCollapsed(c => !c)} style={{
              ...
              color: 'var(--arcilla)', padding: 4, display: isMobile ? 'none' : 'flex'
            }}>
            ...
            <span style={{ fontSize: 14, fontWeight: 500, color: 'var(--tierra)' }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <div style={{
              ...
              fontSize: 11, color: 'var(--musgo)', fontWeight: 500
            }}>
              Campaña 25/26
            </div>
          </div>
        ...
        {isMobile && (
          <nav style={{
            ...
            background: 'var(--blanco)', padding: '8px 0 12px'
          }}>
            {[
              ...
              <button key={item.id} onClick={() => setPage(item.id)} style={{
                ...
                color: page === item.id ? 'var(--pasto)' : 'var(--arcilla)',
                padding: '4px 0'
              }}>
                <div style={{
                  width: 32, height: 32, borderRadius: 8, display: 'flex',
                  alignItems: 'center', justifyContent: 'center',
                  background: page === item.id ? 'var(--verde-light)' : 'transparent'
                }}>
                  <svg width="18" height="18" viewBox="0 0 18 18" fill="none"
                    stroke={page === item.id ? 'var(--pasto)' : 'var(--arcilla)'}
