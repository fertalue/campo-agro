...
export const db = {
  ...
  costos: {
    ...
    eliminar: async (id, eliminado_por) => {
      // Usa función RPC en Supabase — robusto, hace backup y borra en una sola llamada
      const { data, error } = await supabase.rpc('eliminar_costo', {
        p_id: id,
        p_eliminado_por: eliminado_por
      })
      if (error) return { error }
      if (data?.ok === false) return { error: new Error(data.error || 'Error al eliminar') }
      return { data }
    },
    byCentro: (campanha) =>
